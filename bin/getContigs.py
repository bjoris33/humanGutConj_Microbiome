...
for anno in glob.glob("../reassembly_diamond_output/*.m8"):
...
#                name[0] contains the contig WITH the specific read which has the protein of interest
    ...
    if "combined" not in anno:
        fileinput = open(anno, "r")	  # NEEDS TO TAKE IN *.fa FILES, but CONVERT TO .faa.m8 AS YOU NEED THE ORFs FROM .faa FILES LATER, IF CONTIGS FOUND
        matchFound = False
        mainmatch = []
        ...
        fileinput.close()
        fileinput2 = open(anno, "r")	   # NEEDS TO TAKE IN *.fa FILES, but CONVERT TO .faa.m8 AS YOU NEED THE ORFs FROM .faa FILES LATER, IF CONTIGS FOUND
...
#                             fileoutput = open("../conjugative_contigs/" + inname + "a.contigs", "w+")
        ...
        fileinput2.close()
        if matchFound == True:
            fileinput3 = open(anno, "r")      # NEEDS TO TAKE IN *.faa FILES, but CONVERT TO .faa.m8 AS YOU NEED THE ORFs FROM .faa FILES LATER, IF CONTIGS FOUND
            for x in fileinput3:
                name = x.split('\t')
                contig = name[0].rsplit('_', 1)
...
#                     fileoutput.write(x)#"\n" + name[1].strip('\n'))
            ...
            fileinput3.close()
#             fileoutput.close()
    sample_id=anno.split("/")[1].split(".")[0]
    with open("uniref_conjugative_contigs.txt","a") as newfh:
        for i in mainmatch:
            newfh.write(sample_id+"\t"+i+"\n")
